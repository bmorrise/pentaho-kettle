...
define([
  ...
  function appController(dt, fileService, folderService, modalService, searchService, $location, $timeout, $interval, $state, $q) {
    ...
    function onInit() {
      ...
      $timeout(function () {
        ...
        if (!$state.is('selectFolder')) {
          // FIXME is it '$location.search().filter' or '$location.search().filters'
          dt.getDirectoryTree(null).then(function(response) {
    ...
    function selectFolder(folder, useCache) {
      ...
      vm.fileLoading = true;
      vm.folder = folder;
      folderService.selectFolder(folder, null, useCache).then(function(folder) {
        vm.fileLoading = false;
        vm.showRecents = folder.provider === "recents";
        _update();
    ...
    function browserSelectFileObject(file) {
      //FIXME no field file.objectId nor file.objectId.id find out why
    ...
    function browserSelectFileObjectHelper(objectId, name, path, parentPath, connection, provider, type) {
     /*
      * FIXME if an argument is null, it will not send over arguments
