...
define([
  ...
  "./components/utils",
  "css!./app.css"
], function (angular, dataService, fileService, folderService, template, i18n, utils) {
  "use strict";
   ...
   */
  function appController(dt, fs, folderService, $location, $scope, $timeout, $state) {
    var vm = this;
    vm.$onInit = onInit;
    ...
    function onInit() {
      ...
      vm.tree = [
        {name: "Recents", hasChildren: false, provider: "recents"}
      ];
      $timeout(function () {
    ...
    function openFolder(folder, callback) {
      folderService.openFolder(folder, function() {
        if (callback) {
          callback();
        }
      })
    ...
    function selectFolder(folder, callback) {
      ...
      if (folder !== vm.folder) {
        ...
        if (folder.provider === "recents") {
          ...
          vm.fileLoading = true;
          folderService.selectFolder(folder, vm.filters, function() {
            vm.fileLoading = false;
    ...
    function _open(file) {
      try {
        ...
        if (file.repository) {
          ...
          var objectId = file.objectId ? file.objectId.id : null;
          dt.openFile(objectId, file.type, file.path).then(function (response) {
            _closeBrowser();
          });
        }
      }
    ...
    function addDisabled() {
      return false;//addDeleteDisabled() || vm.searchString !== "";
    }
    ...
    function deleteDisabled() {
      return false;//addDeleteDisabled() || isRoot();
    }
    ...
    function getPath(folder) {
      return folderService.getPath(folder);
    }
  }
